...
- **`template/`** 是 `apps/admin` 应用外壳的裁剪副本。`apps/admin` 的 `src/{adapter,layouts,router,store,api/{core,index.ts,request.ts},locales,types}`、 `main.ts`、`bootstrap.ts`、`app.vue`、`preferences.ts`、`index.html`、`public/` 这些文件变了，这里要跟着手工同步。**`views/system`、`api/system`、`api/project.ts`、`views/project` 不应该出现在这里**——第一项属于 `@describeadmin/system-ui`，其余是业务示例，不属于框架外壳。`views/dashboard` 例外：它是登录后必达首页的默认实现，不依赖任何框架实体，刻意作为应用外壳自带的可编辑业务代码随 `apps/admin` 一起同步（同 `views/_core/profile`、`views/_core/about` 的先例），因此**应该**出现在这里。
